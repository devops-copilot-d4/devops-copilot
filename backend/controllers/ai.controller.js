...
const runCopilotDiagnosis = async (req, res, next) => {
  try {
    ...
    if (!telemetry) {
      const error = new Error('Telemetry is required. No diagnosis is generated without authoritative telemetry.');
      error.statusCode = 400;
      error.code = 'TELEMETRY_REQUIRED';
      throw error;
    }
    ...
    const diagnosis = await aiClient.analyzeCopilotState({
      ...
      logs: logs || '',
      events: events || '',
      recentDeploymentInfo: recentDeploymentInfo || '',
    });
